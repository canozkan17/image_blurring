...
int main() 
{
    ...
    // Create threads
    std::thread left_side(thread_blur, &left_args);
    std::thread right_side(thread_blur, &right_args);
    
    // Join threads - wait for both to complete
    left_side.join();
    right_side.join();
    ...
    // Free image data (vectors are automatically managed)
